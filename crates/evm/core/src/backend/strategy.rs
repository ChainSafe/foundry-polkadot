...
use crate::{Env, InspectorExt, backend::JournaledState, evm::new_evm_with_inspector};

use super::{Backend, BackendInner, Fork, ForkDB, FoundryEvmInMemoryDB};
use alloy_evm::{Evm, evm};
use alloy_primitives::Address;
use eyre::{Context, Result};
use revm::{DatabaseRef, context::result::ResultAndState, database::CacheDB};
...
impl BackendStrategyRunner for EvmBackendStrategyRunner {
    fn inspect(
        ...
        let mut evm = new_evm_with_inspector(backend, env.clone(), inspector);

        // TODO: merge.
        let res = evm.transact_raw(env.tx.clone()).wrap_err("EVM error")?;

        let evm = evm;

        env.evm_env.block_env = evm.inner.ctx.block;
        env.evm_env.cfg_env = evm.inner.ctx.cfg;
...
pub(crate) fn merge_account_data<ExtDB: DatabaseRef>(
    ...
    while active_journaled_state.journal.len() > target_fork.journaled_state.journal.len() {
        target_fork.journaled_state.journal.push(revm::JournalEntry::BalanceChange {
            old_balance: Default::default(),
